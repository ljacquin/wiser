...
compute_fixed_effect_vars_declared_as_factors <- function(
  ...
  for (fix_eff_var_ in fixed_effect_vars) {
    if ((length(fixed_effect_vars_computed_as_factor) > 0) &&
      ...
      if ((length(envir_var) > 0 &&
        length(fixed_effect_vars_computed_as_factor_by_envir) > 0) &&
        (fix_eff_var_ %in% fixed_effect_vars_computed_as_factor_by_envir)
      ) {
